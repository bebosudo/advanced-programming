/*
clang++ --std=c++11 custom_date_except.cc -o ../build/custom_date_except &&
        ../build/custom_date_except
*/

#include <iostream>

struct dayInitializerException {
  std::string what() const {
    return "Error: day passed does not exist for the selected month.";
  }
};
...
class CustomDate {
 ...
 public:
  ...
  ~CustomDate() { std::cout << "Date object destroyed\n"; }
};
...
int main(int argc, char const *argv[]) {
  ...
  try {
    CustomDate cd_wrong{2017, Month::feb, 31};
  } catch (const dayInitializerException &e) {
    std::cerr << e.what() << '\n';
    printf("Date February 31, 2017 has not been initialized.\n\n");
  }
...
int CustomDate::days_in_month(Month m) const {
  switch (m) {
    case Month::jan:
      return 31;
    case Month::feb:
